...
volumepath = f"/Volumes/mimi_ws_1/partcd/src/{tablename}" # where all the input files are located
...
files_existing = {x[0] for x in spark.read.table(f"{catalog}.{schema}.hedis_general").select("mimi_src_file_name").distinct().collect()}
...
for filepath in Path(volumepath).glob("*.xlsx"):
    ...
    if filepath.name in files_existing:
        continue
    if filepath.stem.startswith("HEDIS"):
        year2hedis[year] = filepath
    elif filepath.stem.startswith("MADictionary"):
        year2dictionary[year] = filepath
...
for year, hedisfilepath in year2hedis.items():
    
    hedis_lst_sub = []

    madict = pd.read_excel(year2dictionary[year])
    ...
    hedis_measures = pd.read_excel(hedisfilepath, 
    ...
    hedis_measures['rate'] = pd.to_numeric(hedis_measures['rate'], errors='coerce')
    hedis_lst_sub.append(hedis_measures)
    
    hedishos_frm = pd.read_excel(hedisfilepath, 
    ...
    hedishos_pao = pd.read_excel(hedisfilepath, 
    ...
    hedishos_mui = pd.read_excel(hedisfilepath, 
    ...
    hedis_pdf = pd.concat(hedis_lst_sub)
    hedis_pdf["hedis_year"] = int(year)
    hedis_pdf = pd.merge(hedis_pdf, madict, 
    ...
    hedis_pdf["mimi_src_file_date"] = parse(f"{year}-12-31").date()
    hedis_pdf["mimi_src_file_name"] = hedisfilepath.name
    hedis_pdf["mimi_dlt_load_date"] = datetime.today().date()
    ...
    (df.write
        ...
        .option("replaceWhere", f"mimi_src_file_name = '{hedisfilepath.name}'")
...
for year, hedisfilepath in year2hedis.items():
    ...
    madict = madict.loc[:,["measure_code", 
                         ...
                         "measure_name", 
                         "short_indicator_name"]].drop_duplicates()
    hedis_rau_measures = pd.read_excel(hedisfilepath, 
    ...
    hedis_rau_measures["hedis_year"] = int(year)
    hedis_rau_measures = pd.merge(hedis_rau_measures, madict, 
                             on=["measure_code", "indicator_key"], 
                             how="left")
    hedis_rau_pdf = hedis_rau_measures
    hedis_rau_pdf["denominator"] = pd.to_numeric(hedis_rau_pdf["denominator"], 
                                                 errors="coerce")
    hedis_rau_pdf["observed_count"] = pd.to_numeric(hedis_rau_pdf["observed_count"], 
                                                    errors="coerce")
    hedis_rau_pdf["expected_count"] = pd.to_numeric(hedis_rau_pdf["expected_count"], 
                                                    errors="coerce")
    hedis_rau_pdf["member_count"] = pd.to_numeric(hedis_rau_pdf["member_count"], 
                                                  errors="coerce")
    ...
    hedis_rau_pdf["non_outlier_member_count"] = pd.to_numeric(
        hedis_rau_pdf["non_outlier_member_count"], 
        errors="coerce")
    hedis_rau_pdf["count_variance"] = pd.to_numeric(hedis_rau_pdf["count_variance"], 
                                                errors="coerce")
    hedis_rau_pdf["mimi_src_file_date"] = parse(f"{year}-12-31").date()
    hedis_rau_pdf["mimi_src_file_name"] = hedisfilepath.name
    hedis_rau_pdf["mimi_dlt_load_date"] = datetime.today().date()
...
for year, hedisfilepath in year2hedis.items():
    ...
    general = pd.read_excel(hedisfilepath, 
                            sheet_name = "general",
    ...
    general["enrollment_yearend"] = general["enrollment_yearend"].astype(float)
    general["hedis_year"] = int(year)
    general["mimi_src_file_date"] = parse(f"{year}-12-31").date()
    general["mimi_src_file_name"] = hedisfilepath.name
    general["mimi_dlt_load_date"] = datetime.today().date()
    
    df = spark.createDataFrame(general)
    (df.write
        .format('delta')
        .mode("overwrite")
        .option("mergeSchema", "true")
    ...
    national_rates = pd.read_excel(hedisfilepath, 
    ...
    national_rates["mimi_src_file_date"] = parse(f"{year}-12-31").date()
    national_rates["mimi_src_file_name"] = hedisfilepath.name
    national_rates["mimi_dlt_load_date"] = datetime.today().date()
    
    df = spark.createDataFrame(national_rates)
    (df.write
        .format('delta')
        .mode("overwrite")
        .option("mergeSchema", "true")
